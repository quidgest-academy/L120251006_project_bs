...
{
	public class F_movies_ValDatacome_ViewModel : MenuListViewModel<Models.Comme>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<F_movies_ValDatacome_RowViewModel> Menu { get; set; }
		...
		/// <inheritdoc/>
		public override string Uuid => "F_movies_ValDatacome";
		...
		{
// USE /[MANUAL MOV LIST_LIMITS F_MOVIES_PSEUDDATACOME]/
		...
		public F_movies_ValDatacome_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="F_movies_ValDatacome_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public F_movies_ValDatacome_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public F_movies_ValDatacome_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<F_movies_ValDatacome_RowViewModel>();
			...
			if (this.MovieValCodmovie != null)
				crs.Equal(CSGenioAcomme.FldCodmovie, this.MovieValCodmovie);
			...
			{
				// EPH
				crs = Models.Comme.AddEPH<CSGenioAcomme>(ref u, crs, "IBL_F_MOVIESPSEUDDATACOME");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Comme.AddEPH<CSGenioAcomme>(ref u, null, "IBL_F_MOVIESPSEUDDATACOME"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcomme> Qlisting, ref CriteriaSet conditions)
		{
				...
				CriteriaSet f_moviespseuddatacomeConds = CriteriaSet.And();
				...
				//Tooltip for EPHs affecting this viewmodel list
				{
					...
					List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_F_MOVIESPSEUDDATACOME");
				...
				conditions.SubSets.Add(f_moviespseuddatacomeConds);
				f_moviespseuddatacomeConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
				tableReload &= hasAllRequiredLimits;

// USE /[MANUAL MOV OVERRQ F_MOVIES_PSEUDDATACOME]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAcomme>(m_userContext, false, f_moviespseuddatacomeConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_F_MOVIESPSEUDDATACOME", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL MOV OVERRQLSTEXP F_MOVIES_PSEUDDATACOME]/
				...
				if (tableReload)
				{
// USE /[MANUAL MOV OVERRQLIST F_MOVIES_PSEUDDATACOME]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAcomme.GetInformation(), QMVC_POS_RECORD, sorts, f_moviespseuddatacomeConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAcomme> listing = Models.ModelBase.Where<CSGenioAcomme>(m_userContext, distinct, f_moviespseuddatacomeConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_F_MOVIESPSEUDDATACOME", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
					...
					Menu.Elements = MapF_movies_ValDatacome(listing);

					Menu.Identifier = "IBL_F_MOVIESPSEUDDATACOME";

					// Last updated by [CJP] at [2015.02.03]
					// Adds the identifier to each element
					foreach (var element in Menu.Elements)
						element.Identifier = "IBL_F_MOVIESPSEUDDATACOME";
		...
		private List<F_movies_ValDatacome_RowViewModel> MapF_movies_ValDatacome(ListingMVC<CSGenioAcomme> Qlisting)
		{
			List<F_movies_ValDatacome_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapF_movies_ValDatacome(row));
					i++;
				}
			}
		...
		/// to a F_movies_ValDatacome_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private F_movies_ValDatacome_RowViewModel MapF_movies_ValDatacome(CSGenioAcomme row)
		{
			var model = new F_movies_ValDatacome_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL MOV VIEWMODEL_CUSTOM F_MOVIES_VALDATACOME]/
