...
{
	public class F_rattin_MovieValTitle_ViewModel : MenuListViewModel<Models.Movie>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<F_rattin_MovieValTitle_RowViewModel> Menu { get; set; }
		...
		/// <inheritdoc/>
		public override string Uuid => "F_rattin_MovieValTitle";
		...
		{
// USE /[MANUAL MOV LIST_LIMITS F_RATTIN_MOVIETITLE]/
		...
		public F_rattin_MovieValTitle_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="F_rattin_MovieValTitle_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public F_rattin_MovieValTitle_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public F_rattin_MovieValTitle_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<F_rattin_MovieValTitle_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Movie.AddEPH<CSGenioAmovie>(ref u, crs, "IBL_F_RATTINMOVIETITLE___");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Movie.AddEPH<CSGenioAmovie>(ref u, null, "IBL_F_RATTINMOVIETITLE___"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAmovie> Qlisting, ref CriteriaSet conditions)
		{
				...
				//Tooltip for EPHs affecting this viewmodel list
				{
					...
					List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_F_RATTINMOVIETITLE___");
				...
				conditions.SubSets.Add(f_rattinmovietitle___Conds);
				f_rattinmovietitle___Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL MOV OVERRQ F_RATTIN_MOVIETITLE]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAmovie>(m_userContext, false, f_rattinmovietitle___Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_F_RATTINMOVIETITLE___", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL MOV OVERRQLSTEXP F_RATTIN_MOVIETITLE]/
				...
				if (tableReload)
				{
// USE /[MANUAL MOV OVERRQLIST F_RATTIN_MOVIETITLE]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAmovie.GetInformation(), QMVC_POS_RECORD, sorts, f_rattinmovietitle___Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAmovie> listing = Models.ModelBase.Where<CSGenioAmovie>(m_userContext, distinct, f_rattinmovietitle___Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_F_RATTINMOVIETITLE___", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
					...
					Menu.Elements = MapF_rattin_MovieValTitle(listing);

					Menu.Identifier = "IBL_F_RATTINMOVIETITLE___";

					// Last updated by [CJP] at [2015.02.03]
					// Adds the identifier to each element
					foreach (var element in Menu.Elements)
						element.Identifier = "IBL_F_RATTINMOVIETITLE___";
		...
		private List<F_rattin_MovieValTitle_RowViewModel> MapF_rattin_MovieValTitle(ListingMVC<CSGenioAmovie> Qlisting)
		{
			List<F_rattin_MovieValTitle_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapF_rattin_MovieValTitle(row));
					i++;
				}
			}
		...
		/// to a F_rattin_MovieValTitle_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private F_rattin_MovieValTitle_RowViewModel MapF_rattin_MovieValTitle(CSGenioAmovie row)
		{
			var model = new F_rattin_MovieValTitle_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL MOV VIEWMODEL_CUSTOM F_RATTIN_MOVIEVALTITLE]/
