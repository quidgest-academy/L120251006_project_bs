...
{
	public class MOV_Menu_41_ViewModel : MenuListViewModel<Models.Chara>
	{
		/// <summary>
...
// USE /[MANUAL MOV LIST_LIMITS 41]/
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (!tableConfig.StaticFilters.ContainsKey("filter_MOV_Menu_41_GENREFILTE"))
				tableConfig.StaticFilters.Add("filter_MOV_Menu_41_GENREFILTE", null);

			{
				var groupFilters = CriteriaSet.Or();
				bool filter_MOV_Menu_41_GENREFILTE_1 = false;
				if (tableConfig.StaticFilters["filter_MOV_Menu_41_GENREFILTE"] != null)
					filter_MOV_Menu_41_GENREFILTE_1 = tableConfig.StaticFilters["filter_MOV_Menu_41_GENREFILTE"].Contains("1");
				if (filter_MOV_Menu_41_GENREFILTE_1)
				{
					groupFilters.Equal(CSGenioAchara.FldGenre, "M");

				}

				bool filter_MOV_Menu_41_GENREFILTE_2 = false;
				if (tableConfig.StaticFilters["filter_MOV_Menu_41_GENREFILTE"] != null)
					filter_MOV_Menu_41_GENREFILTE_2 = tableConfig.StaticFilters["filter_MOV_Menu_41_GENREFILTE"].Contains("2");
				if (filter_MOV_Menu_41_GENREFILTE_2)
				{
					groupFilters.Equal(CSGenioAchara.FldGenre, "F");

				}

				bool filter_MOV_Menu_41_GENREFILTE_3 = false;
				if (tableConfig.StaticFilters["filter_MOV_Menu_41_GENREFILTE"] != null)
					filter_MOV_Menu_41_GENREFILTE_3 = tableConfig.StaticFilters["filter_MOV_Menu_41_GENREFILTE"].Contains("3");
				if (filter_MOV_Menu_41_GENREFILTE_3)
				{
					groupFilters.Equal(CSGenioAchara.FldGenre, "O");

				}

				bool filter_MOV_Menu_41_GENREFILTE_4 = false;
				if (tableConfig.StaticFilters["filter_MOV_Menu_41_GENREFILTE"] != null)
					filter_MOV_Menu_41_GENREFILTE_4 = tableConfig.StaticFilters["filter_MOV_Menu_41_GENREFILTE"].Contains("4");
				if (filter_MOV_Menu_41_GENREFILTE_4)
				{
			...
			{
				// EPH
				crs = Models.Chara.AddEPH<CSGenioAchara>(ref u, crs, "ML41");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Chara.AddEPH<CSGenioAchara>(ref u, null, "ML41"));
			}
...
// USE /[MANUAL MOV OVERRQ 41]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAchara>(m_userContext, false, mov_menu_41Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML41", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL MOV OVERRQLIST 41]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAchara.GetInformation(), QMVC_POS_RECORD, sorts, mov_menu_41Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAchara> listing = Models.ModelBase.Where<CSGenioAchara>(m_userContext, distinct, mov_menu_41Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML41", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		private List<MOV_Menu_41_RowViewModel> MapMOV_Menu_41(ListingMVC<CSGenioAchara> Qlisting)
		{
		...
		private MOV_Menu_41_RowViewModel MapMOV_Menu_41(CSGenioAchara row)
		{
