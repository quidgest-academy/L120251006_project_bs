...
namespace GenioMVC.Controllers
{
	public partial class RattiController : ControllerBase
	{
		...
		public RattiController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MOV CONTROLLER_NAVIGATION RATTI]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAratti>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MOV MANUAL_CONTROLLER RATTI]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Ratti row = new Models.Ratti(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("ratti");
			...
			try
			{
				...
				{
					case "F_RATTINMOVIETITLE___":	// Field (DB)
						{
							var model = new F_rattin_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_rattinmovietitle___(qs);
							result = model.TableMovieTitle;
						}
						break;
					case "F_RATTINUSERPNAME____":	// Field (DB)
						{
							var model = new F_rattin_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							// Map received value to field - The non-duplication prefix field for foreign key field with the unique value
							model.SetViewModelValue("ratti.codmovie", Navigation.GetValue<string>("ratti.codmovie"));
							model.Load_F_rattinuserpname____(qs);
							result = model.TableUserpName;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_RATTINMOVIETITLE___":	// Field (DB)
						values = new F_rattin_ViewModel(UserContext.Current).GetDependant_F_rattinTableMovieTitle(Selected);
						break;
					case "F_RATTINUSERPNAME____":	// Field (DB)
						values = new F_rattin_ViewModel(UserContext.Current).GetDependant_F_rattinTableUserpName(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_rattin" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_rattin([FromBody]F_rattin_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "ratti",
				(primaryKey) => Models.Ratti.Find(primaryKey, UserContext.Current, "FF_RATTIN"),
				(model) => formData.MapToModel(model as Models.Ratti)
			);
		}
		...
		{
			return base.GetDocumsTickets("RATTI", requestModel.FieldName, requestModel.KeyValue);
		}
