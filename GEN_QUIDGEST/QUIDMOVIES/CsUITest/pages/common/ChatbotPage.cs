...
public class ChatbotPage: PageObject
{
    ...
    /// Clear chat button
    /// </summary>
    private ButtonControl ClearButton => new ButtonControl(driver, toolbarContainerLocator, "button");
    ...
    public string SendMessage(string message)
    {
        ...
        WebDriverWait waitForStart = new WebDriverWait(driver, TimeSpan.FromSeconds(5));
        waitForStart.Until(c => generatingResponse);

        // Then wait for the notice about generating a response to disappear
        WebDriverWait waitForFinish = new WebDriverWait(driver, TimeSpan.FromSeconds(5));
