<template>
	...
	<qtable 
		...
		<template #MOV="props">
			<q-badge
				v-for="userRole in props.row.UserRoles.filter(role => role.Module === 'MOV' && Modules[role.Module].active)"
				class="q-table__cell--multiple"
				:key="userRole.Role"
				variant="bold" >
				{{ Resources[userRole.Designation] }}
	...
	export default {
		...
		data() {
			...
			return {
				...
				userId: '',
				Modules: {
					MOV : { active: true, Cod: "MOV", Description: 'MY_APPLICATION56216'},
				},
				Users: {
					rows: [],
					total_rows: 0,
		...
		computed: {
			selectModules: {
                get() {
					...
					return                     this.Modules.MOV.active ;
                },
                set(value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.MOV.active = value;
                    return value;
                }
            },
		...
		methods: {
			...
			userColumns() {
				...
				{
					label: () => vm.$t(vm.Modules.MOV.Description),
					name: "MOV",
					slot_name: "MOV",
					sort: false,
					visibility: vm.Modules.MOV.active && vm.columnModules
				},
			];
		},
		ImportarUsersAD() {
