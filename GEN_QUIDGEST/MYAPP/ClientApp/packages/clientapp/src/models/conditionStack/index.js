...
class ConditionStack
{
	...
	{
		if (typeof processMonitor !== 'undefined' && !(processMonitor instanceof QAsyncProcessMonitor))
			throw new TypeError('The "processMonitor" argument must be an instance of QAsyncProcessMonitor.')
		this.processMonitor = processMonitor
	}
	...
	async add(sourceId, condition, eventIds)
	{
		if (typeof sourceId !== 'string' || sourceId.length === 0)
			throw new TypeError('An invalid source was specified, the value should be a non-empty string.')

		if (sourceId in this.sources)
			return false

		const source = await ConditionSource.createSource(condition, eventIds, this.globalEvents, this.isMetVal, this.processMonitor)
