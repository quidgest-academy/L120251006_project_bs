...
namespace GenioMVC.Controllers
{
	public partial class UserpController : ControllerBase
	{
		...
		public UserpController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MOV CONTROLLER_NAVIGATION USERP]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAuserp>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL MOV MANUAL_CONTROLLER USERP]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Userp row = new Models.Userp(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("userp");
			...
			try
			{
				...
				{
					case "F_USER__PSW__NOME____":	// Field (DB)
						{
							var model = new F_user_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_user__psw__nome____(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_USER__PSW__NOME____":	// Field (DB)
						values = new F_user_ViewModel(UserContext.Current).GetDependant_F_userTablePswNome(Selected);
						break;
					default: break;
				}
		...
		{
			return base.GetDocumsTickets("USERP", requestModel.FieldName, requestModel.KeyValue);
		}
