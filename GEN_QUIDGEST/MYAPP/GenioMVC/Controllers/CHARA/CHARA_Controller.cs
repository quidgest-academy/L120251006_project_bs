...
namespace GenioMVC.Controllers
{
	public partial class CharaController : ControllerBase
	{
		...
		public CharaController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MOV CONTROLLER_NAVIGATION CHARA]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAchara>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MOV MANUAL_CONTROLLER CHARA]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Chara row = new Models.Chara(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("chara");
			...
			try
			{
				...
				{
					case "F_CHARACMOVIETITLE___":	// Field (DB)
						{
							var model = new F_charac_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_characmovietitle___(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_CHARACMOVIETITLE___":	// Field (DB)
						values = new F_charac_ViewModel(UserContext.Current).GetDependant_F_characTableMovieTitle(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_charac" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_charac([FromBody]F_charac_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "chara",
				(primaryKey) => Models.Chara.Find(primaryKey, UserContext.Current, "FF_CHARAC"),
				(model) => formData.MapToModel(model as Models.Chara)
			);
		}
		...
		{
			return base.GetDocumsTickets("CHARA", requestModel.FieldName, requestModel.KeyValue);
		}
