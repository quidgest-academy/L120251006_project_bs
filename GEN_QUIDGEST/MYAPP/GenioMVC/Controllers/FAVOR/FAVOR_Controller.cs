...
namespace GenioMVC.Controllers
{
	public partial class FavorController : ControllerBase
	{
		...
		public FavorController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MOV CONTROLLER_NAVIGATION FAVOR]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAfavor>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MOV MANUAL_CONTROLLER FAVOR]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Favor row = new Models.Favor(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("favor");
			...
			try
			{
				...
				{
					case "F_FAVORIUSERPNAME____":	// Field (DB)
						{
							var model = new F_favori_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_favoriuserpname____(qs);
							result = model.TableUserpName;
						}
						break;
					case "F_FAVORIMOVIETITLE___":	// Field (DB)
						{
							var model = new F_favori_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_FAVORIUSERPNAME____":	// Field (DB)
						values = new F_favori_ViewModel(UserContext.Current).GetDependant_F_favoriTableUserpName(Selected);
						break;
					case "F_FAVORIMOVIETITLE___":	// Field (DB)
						values = new F_favori_ViewModel(UserContext.Current).GetDependant_F_favoriTableMovieTitle(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_favori" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_favori([FromBody]F_favori_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "favor",
				(primaryKey) => Models.Favor.Find(primaryKey, UserContext.Current, "FF_FAVORI"),
				(model) => formData.MapToModel(model as Models.Favor)
			);
		}
		...
		{
			return base.GetDocumsTickets("FAVOR", requestModel.FieldName, requestModel.KeyValue);
		}
