...
namespace GenioMVC.Controllers
{
	public partial class CommeController : ControllerBase
	{
		...
		public CommeController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MOV CONTROLLER_NAVIGATION COMME]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAcomme>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MOV MANUAL_CONTROLLER COMME]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Comme row = new Models.Comme(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("comme");
			...
			try
			{
				...
				{
					case "F_COMMENUSERPNAME____":	// Field (DB)
						{
							var model = new F_commen_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_commenuserpname____(qs);
							result = model.TableUserpName;
						}
						break;
					case "F_COMMENMOVIETITLE___":	// Field (DB)
						{
							var model = new F_commen_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_commenmovietitle___(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_COMMENUSERPNAME____":	// Field (DB)
						values = new F_commen_ViewModel(UserContext.Current).GetDependant_F_commenTableUserpName(Selected);
						break;
					case "F_COMMENMOVIETITLE___":	// Field (DB)
						values = new F_commen_ViewModel(UserContext.Current).GetDependant_F_commenTableMovieTitle(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_commen" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_commen([FromBody]F_commen_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "comme",
				(primaryKey) => Models.Comme.Find(primaryKey, UserContext.Current, "FF_COMMEN"),
				(model) => formData.MapToModel(model as Models.Comme)
			);
		}
		...
		{
			return base.GetDocumsTickets("COMME", requestModel.FieldName, requestModel.KeyValue);
		}
