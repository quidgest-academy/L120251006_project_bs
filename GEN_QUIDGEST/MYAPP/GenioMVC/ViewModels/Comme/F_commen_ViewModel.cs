...
{
	public class F_commen_ViewModel : FormViewModel<Models.Comme>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public F_commen_ViewModel() : base(null!) { }

		public F_commen_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FF_COMMEN", nestedForm) { }

		public F_commen_ViewModel(UserContext userContext, Models.Comme row, bool nestedForm = false) : base(userContext, "FF_COMMEN", row, nestedForm) { }

		public F_commen_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("comme", id);
			Model = Models.Comme.Find(id, userContext, "FF_COMMEN", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Comme model = new Models.Comme(userContext) { Identifier = "FF_COMMEN" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FF_COMMEN");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Comme m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Comme) to ViewModel (F_commen) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Comme) to ViewModel (F_commen) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Comme m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (F_commen) to Model (Comme) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (F_commen) to Model (Comme) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "comme.codcomme":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (F_commen) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (F_commen)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Comme.Find(id ?? Navigation.GetStrValue("comme"), m_userContext, "FF_COMMEN"); }
			finally { Model ??= new Models.Comme(m_userContext) { Identifier = "FF_COMMEN" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Comme.Find(Navigation.GetStrValue("comme"), m_userContext, "FF_COMMEN");
			}
			finally
			{
				if (Model == null)
		...
		protected override void LoadDocumentsProperties(Models.Comme row)
		{
		}
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Comme(m_userContext) { Identifier = "FF_COMMEN" };
			...
			Load_F_commenuserpname____(qs, lazyLoad);
			Load_F_commenmovietitle___(qs, lazyLoad);
// USE /[MANUAL MOV VIEWMODEL_LOADPARTIAL F_COMMEN]/
		}

// USE /[MANUAL MOV VIEWMODEL_NEW F_COMMEN]/
...
// USE /[MANUAL MOV VIEWMODEL_SAVE F_COMMEN]/
...
// USE /[MANUAL MOV VIEWMODEL_APPLY F_COMMEN]/

// USE /[MANUAL MOV VIEWMODEL_DUPLICATE F_COMMEN]/

// USE /[MANUAL MOV VIEWMODEL_DESTROY F_COMMEN]/
		public override void Destroy(string id)
		{
			Model = Models.Comme.Find(id, m_userContext, "FF_COMMEN");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_F_commenuserpname____(NameValueCollection qs, bool lazyLoad = false)
		{
			bool f_commenuserpname____DoLoad = true;
			CriteriaSet f_commenuserpname____Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("userp", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					f_commenuserpname____Conds.Equal(CSGenioAuserp.FldCoduserp, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_F_commenTableUserpName(lazyLoad);
				return;
			}

			if (f_commenuserpname____DoLoad)
			{
				...
				f_commenuserpname____Conds.SubSet(search_filters);
...
// USE /[MANUAL MOV OVERRQ F_COMMEN_USERPNAME]/
				...
				*/
				if (Navigation.checkFormMode("userp", FormMode.New) || Navigation.checkFormMode("userp", FormMode.Duplicate))
					f_commenuserpname____Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAuserp.FldZzstate, 0)
						.Equal(CSGenioAuserp.FldCoduserp, Navigation.GetStrValue("userp")));
				else
					f_commenuserpname____Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAuserp.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("userp", "name");
				ListingMVC<CSGenioAuserp> listing = Models.ModelBase.Where<CSGenioAuserp>(m_userContext, false, f_commenuserpname____Conds, fields, offset, numberItems, sorts, "LED_F_COMMENUSERPNAME____", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableUserpName.Elements = listing.RowsForViewModel<GenioMVC.Models.Userp>((r) => new GenioMVC.Models.Userp(m_userContext, r, true, _fieldsToSerialize_F_COMMENUSERPNAME____));
				...
				FillDependant_F_commenTableUserpName();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_F_commenTableUserpName(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_F_commenTableUserpName(bool lazyLoad = false)
		{
			var row = GetDependant_F_commenTableUserpName(this.ValCoduserp);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_F_COMMENUSERPNAME____ = ["Userp", "Userp.ValCoduserp", "Userp.ValZzstate", "Userp.ValName"];
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_F_commenmovietitle___(NameValueCollection qs, bool lazyLoad = false)
		{
			bool f_commenmovietitle___DoLoad = true;
			CriteriaSet f_commenmovietitle___Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("movie", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					f_commenmovietitle___Conds.Equal(CSGenioAmovie.FldCodmovie, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_F_commenTableMovieTitle(lazyLoad);
				return;
			}

			if (f_commenmovietitle___DoLoad)
			{
				List<ColumnSort> sorts = new List<ColumnSort>();
				...
				f_commenmovietitle___Conds.SubSet(search_filters);
...
// USE /[MANUAL MOV OVERRQ F_COMMEN_MOVIETITLE]/
				...
				*/
				if (Navigation.checkFormMode("movie", FormMode.New) || Navigation.checkFormMode("movie", FormMode.Duplicate))
					f_commenmovietitle___Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAmovie.FldZzstate, 0)
						.Equal(CSGenioAmovie.FldCodmovie, Navigation.GetStrValue("movie")));
				else
					f_commenmovietitle___Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAmovie.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("movie", "title");
				ListingMVC<CSGenioAmovie> listing = Models.ModelBase.Where<CSGenioAmovie>(m_userContext, false, f_commenmovietitle___Conds, fields, offset, numberItems, sorts, "LED_F_COMMENMOVIETITLE___", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableMovieTitle.Elements = listing.RowsForViewModel<GenioMVC.Models.Movie>((r) => new GenioMVC.Models.Movie(m_userContext, r, true, _fieldsToSerialize_F_COMMENMOVIETITLE___));
				...
				FillDependant_F_commenTableMovieTitle();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_F_commenTableMovieTitle(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_F_commenTableMovieTitle(bool lazyLoad = false)
		{
			var row = GetDependant_F_commenTableMovieTitle(this.ValCodmovie);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_F_COMMENMOVIETITLE___ = ["Movie", "Movie.ValCodmovie", "Movie.ValZzstate", "Movie.ValTitle"];
...
// USE /[MANUAL MOV VIEWMODEL_CUSTOM F_COMMEN]/
